...
+	      DIRFILESEP DFSEP
 	      ...
 	      VERSION_SYM XS_VERSION INST_BIN INST_EXE INST_LIB
 	      INST_ARCHLIB INST_SCRIPT PREFIX  INSTALLDIRS
@@ -1026,7 +1030,7 @@ BOOTSTRAP = '."$self->{BASEEXT}.bs".'
 # As Mkbootstrap might not write a file (if none is required)
 # we use touch to prevent make continually trying to remake it.
 # The DynaLoader only reads a non-empty file.
-$(BOOTSTRAP): '."$self->{MAKEFILE} $self->{BOOTDEP}".' $(INST_ARCHAUTODIR)/.exists
+$(BOOTSTRAP): '."$self->{MAKEFILE} $self->{BOOTDEP}".' $(INST_ARCHAUTODIR)$(DFSEP).exists
 	...
 	'.$self->{NOECHO}.'$(PERL) "-I$(PERL_ARCHLIB)" "-I$(PERL_LIB)" \
 		-MExtUtils::Mkbootstrap \
@@ -1034,7 +1038,7 @@ $(BOOTSTRAP): '."$self->{MAKEFILE} $self->{BOOTDEP}".' $(INST_ARCHAUTODIR)/.exis
 	'.$self->{NOECHO}.'$(TOUCH) $(BOOTSTRAP)
 	$(CHMOD) $(PERM_RW) $@
...
+$(INST_DYNAMIC): $(OBJECT) $(MYEXTLIB) $(BOOTSTRAP) $(INST_ARCHAUTODIR)$(DFSEP).exists $(EXPORT_LIST) $(PERL_ARCHIVE) $(PERL_ARCHIVE_AFTER) $(INST_DYNAMIC_DEP)
 ');
     if ($armaybe ne ':'){
 	$ldfrom = 'tmp$(LIB_EXT)';
@@ -2024,6 +2028,20 @@ usually solves this kind of problem.
     # Define 'FULLPERL' to be a non-miniperl (used in test: target)
     ($self->{FULLPERL} = $self->{PERL}) =~ s/miniperl/perl/i
 	unless ($self->{FULLPERL});
+
...
+    $self->{DIRFILESEP} = '/';
 ...
 =item init_others
@@ -2494,7 +2512,7 @@ MAP_LIBPERL = $libperl
 ";
...
+$(INST_STATIC): $(OBJECT) $(MYEXTLIB) $(INST_ARCHAUTODIR)$(DFSEP).exists
 ...
 END
     # If this extension has it's own library (eg SDBM_File)
@@ -3672,13 +3690,13 @@ pure_all :: config pm_to_blib subdirs linkext
 subdirs :: $(MYEXTLIB)
 	'.$self->{NOECHO}.'$(NOOP)
...
@@ -3686,7 +3704,7 @@ config :: $(INST_AUTODIR)/.exists
...
@@ -3695,7 +3713,7 @@ config :: \$(INST_HTMLLIBDIR)/.exists
...
@@ -3704,7 +3722,7 @@ config :: \$(INST_HTMLSCRIPTDIR)/.exists
...
@@ -3712,7 +3730,7 @@ config :: \$(INST_MAN1DIR)/.exists
     }
     if (%{$self->{MAN3PODS}}) {
 	push @m, qq[
-config :: \$(INST_MAN3DIR)/.exists
...
+		    PL_my_ctx_mutex
 		    ...
 		    Perl_cx_dup
 		    Perl_si_dup
@@ -753,7 +754,6 @@ unless ($define{'USE_ITHREADS'}) {
...
-		    PL_my_ctx_mutex
 		    ...
 		    PL_my_cxt_list
 		    PL_my_cxt_size
@@ -1129,6 +1129,10 @@ if ($define{'MULTIPLICITY'}) {
 	my $glob = readvar($f, sub { "Perl_" . $_[1] . $_[2] . "_ptr" });
 	emit_symbols $glob;
     }
+    unless ($define{'USE_ITHREADS'}) {
...
--- perl.c
+++ perl.c
@@ -166,8 +166,6 @@ S_init_tls_and_interp(PerlInterpreter *my_perl)
 	OP_REFCNT_INIT;
 	HINTS_REFCNT_INIT;
 	MUTEX_INIT(&PL_dollarzero_mutex);
-#  endif
...
--- util.c
+++ util.c
@@ -5506,9 +5506,13 @@ Perl_my_cxt_init(pTHX_ int *index, size_t size)
     void *p;
     if (*index == -1) {
 	/* this module hasn't been allocated an index yet */
+#if defined(USE_ITHREADS)
...
--- makedef.pl
+++ makedef.pl
@@ -1089,6 +1089,10 @@ if ($define{'MULTIPLICITY'}) {
 	my $glob = readvar($f, sub { "Perl_" . $_[1] . $_[2] . "_ptr" });
 	emit_symbols $glob;
     }
+    unless ($define{'USE_ITHREADS'}) {
