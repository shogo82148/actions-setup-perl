...
+			(cd "$dir" && . ./$file)
 		...
 		*)
 			. ./$file
@@ -1489,19 +1531,19 @@ for file in $*; do
 			dir=`expr X$file : 'X\(.*\)/'`
 			file=`expr X$file : 'X.*/\(.*\)'`
 			(set x $dir; shift; eval $mkdir_p)
-			sh <$src/$dir/$file
...
+		(cd "$rsrc"; ls `cat "$tmppwd/$filelist"` >/dev/null 2>>"$tmppwd/missing")
 	...
 	if test -s missing; then
 		cat missing >&4
@@ -1612,6 +1654,11 @@ if test X"$trnl" = X; then
 	foox) trnl='\012' ;;
 	esac
 fi
+if test X"$trnl" = X; then
...
@@ -1950,7 +1997,7 @@ for file in $trylist; do
 	'') xxx=`./loc $file $file $pth`;;
 	*) xxx=`./loc $xxx $xxx $pth`;;
 	esac
-	eval $file=$xxx
...
+		elif $test X"$tans" = X -o X"$tans" = Xnone ; then
 			...
 			: Give one chance to correct a possible typo.
@@ -3065,7 +3356,7 @@ if $test -f cc.cbu; then
 fi
 echo " "
 echo "Checking for GNU cc in disguise and/or its version number..." >&4
-$cat >gccvers.c <<EOM
...
@@ -3756,7 +4047,7 @@ for thislib in $libswanted; do
 	for thisdir in $libspath; do
 	    xxx=''
 	    if $test ! -f "$xxx" -a "X$ignore_versioned_solibs" = "X"; then
-		xxx=`ls $thisdir/lib$thislib.$so.[0-9] 2>/dev/null|tail -1`
...
+	lseeksize=`$run ./try`
 	...
 	dflt=$longsize
@@ -4305,6 +4716,10 @@ echo "Checking the size of $zzz..." >&4
 cat > try.c <<EOCP
 #include <sys/types.h>
 #include <stdio.h>
+#$i_stdlib I_STDLIB
...
+		yyy=`$run ./try`
 		...
 		123.456)
 			sPRIfldbl='"lf"'; sPRIgldbl='"lg"'; sPRIeldbl='"le"';
@@ -7557,9 +7999,13 @@ char *myname = "qgcvt";
 #define DOUBLETYPE long double
 #endif
 #ifdef TRY_sprintf
-#if defined(USE_LONG_DOUBLE) && defined(HAS_LONG_DOUBLE) && defined(HAS_PRIgldbl)
...
+	$cat >access.c <<EOCP
 ...
 #ifdef I_FCNTL
 #include <fcntl.h>
@@ -7734,6 +8180,10 @@ case "$d_access" in
 #ifdef I_UNISTD
 #include <unistd.h>
 #endif
+#$i_stdlib I_STDLIB
...
+void croak (char* pat,...) __attribute__((__format__(__printf__,1,2),noreturn));
 ...
 if $cc $ccflags -c attrib.c >attrib.out 2>&1 ; then
 	if $contains 'warning' attrib.out >/dev/null 2>&1; then
@@ -7816,12 +8266,16 @@ case "$d_getpgrp" in
 "$define")
 	echo " "
 	echo "Checking to see which flavor of getpgrp is in use..."
-	$cat >set.c <<EOP
...
+$rm -f try try.*
 ...
 : see if setpgrp exists
 set setpgrp d_setpgrp
@@ -7878,12 +8332,16 @@ case "$d_setpgrp" in
 "$define")
 	echo " "
 	echo "Checking to see which flavor of setpgrp is in use..."
-	$cat >set.c <<EOP
...
+$cat >try.c <<EOCP
 ...
 #ifdef I_FCNTL
 #include <fcntl.h>
@@ -8549,6 +9030,10 @@ $cat >open3.c <<'EOCP'
 #ifdef I_SYS_FILE
 #include <sys/file.h>
 #endif
+#$i_stdlib I_STDLIB
...
@@ -8571,10 +9056,10 @@ if $test `./findhdr sys/file.h` && \
 		val="$undef"
 	fi
 elif $test `./findhdr fcntl.h` && \
-		set open3 -DI_FCNTL && eval $compile; then
...
+$rm -f try try.*
 ...
 : see which of string.h or strings.h is needed
 echo " "
@@ -8611,6 +9096,35 @@ case "$i_string" in
 *)	  strings=`./findhdr string.h`;;
 esac
...
+
 ...
 case "$h_sysfile" in
 true) echo "#include <sys/file.h>" > head.c;;
@@ -8626,8 +9140,16 @@ echo "Figuring out the flag used by open() for non-blocking I/O..." >&4
 case "$o_nonblock" in
 '')
 	$cat head.c > try.c
-	$cat >>try.c <<'EOCP'
...
+	$cat >try.c <<EOCP
 ...
 #ifdef I_TIME
 #include <time.h>
@@ -8936,6 +9466,10 @@ if test "X$timeincl" = X; then
 #ifdef I_SYSSELECT
 #include <sys/select.h>
 #endif
+#$i_stdlib I_STDLIB
...
+		longlongsize=`$run ./try`
 		...
 		dflt='8'
@@ -9949,12 +10491,7 @@ case "$quadtype" in
 '')	echo "Alas, no 64-bit integer types in sight." >&4
 	d_quad="$undef"
 	;;
-*)	if test X"$use64bitint" = Xdefine -o X"$longsize" = X8; then
...
+		case "`$run ./try`" in
 		int32_t)
 			i32type=int32_t
 			u32type=uint32_t
@@ -11170,6 +11711,10 @@ if set sigaction val -f d_sigaction; eval $csym; $val; then
 	echo 'sigaction() found.' >&4
 	$cat > try.c <<'EOP'
 #include <stdio.h>
+#$i_stdlib I_STDLIB
...
+#endif
 ...
 #include <signal.h>
 int main()
@@ -11199,8 +11744,12 @@ $rm -f try try$_o try.c
 echo " "
 case "$d_sigsetjmp" in
 '')
-	$cat >try.c <<'EOP'
...
@@ -11388,6 +11961,10 @@ case "$d_stdstdio" in
 $define)
 	$cat >try.c <<EOP
 #include <stdio.h>
+#$i_stdlib I_STDLIB
...
+
 ...
 set strxfrm d_strxfrm
 eval $inlibc
@@ -11780,7 +12405,7 @@ case "$d_closedir" in
 "$define")
 	echo " "
 	echo "Checking whether closedir() returns a status..." >&4
-	cat > closedir.c <<EOM
+	cat > try.c <<EOM
 ...
 #$i_sysdir I_SYS_DIR		/**/
 #$i_sysndir I_SYS_NDIR		/**/
@@ -11809,9 +12434,9 @@ case "$d_closedir" in
 #endif 
 int main() { return closedir(opendir(".")); }
 EOM
-	set closedir
 ...
 *)
 	case "$byteorder" in
@@ -11977,21 +12602,27 @@ an Alpha will report 12345678. If the test program works the default is
 probably right.
 I'm now running the test program...
 EOM
-		$cat >try.c <<'EOCP'
...
+$cat >try.c <<'EOM'
 ...
 {
   if ('M'==0xd4) return 0;
@@ -12492,6 +13131,10 @@ sunos) $echo '#define PERL_FFLUSH_ALL_FOPEN_MAX 32' > try.c ;;
 esac
 $cat >>try.c <<EOCP
 #include <stdio.h>
+#$i_stdlib I_STDLIB
...
+#endif
 ...
 #ifdef I_UNISTD
 # include <unistd.h>
@@ -12502,7 +13145,9 @@ $cat >>try.c <<EOCP
 # define STDIO_STREAM_ARRAY $stdio_stream_array
 #endif
 int main() {
-  FILE* p = fopen("try.out", "w");
...
@@ -12803,6 +13451,10 @@ echo "Checking the size of $zzz..." >&4
 cat > try.c <<EOCP
 #include <sys/types.h>
 #include <stdio.h>
+#$i_stdlib I_STDLIB
...
@@ -13372,8 +14024,12 @@ case "$ptrsize" in
 	else
 		echo '#define VOID_PTR void *' > try.c
 	fi
-	$cat >>try.c <<'EOCP'
...
+	$awk "{print \\$$fieldn}" | $sed 's!"!!g' | $sed 's!\\\\\\\\!/!g' | $sort | $uniq`
 ...
 : Check this list of files to be sure we have parsed the cpp output ok.
 : This will also avoid potentially non-existent files, such 
@@ -13627,9 +14324,13 @@ xxx="$xxx SYS TERM THAW TRAP TSTP TTIN TTOU URG USR1 USR2"
 xxx="$xxx USR3 USR4 VTALRM WAITING WINCH WIND WINDOW XCPU XFSZ"
...
+($1 !~ /^NSIG$/) && (NF == 2) && ($2 ~ /^[0-9][0-9]*$/) {
     ...
     if (sig_name[$2]) {
 	dup_name[ndups] = $1
@@ -13746,13 +14447,13 @@ $cat >>signal_cmd <<'EOS'
...
@@ -13856,6 +14557,10 @@ echo "Checking the size of $zzz..." >&4
 cat > try.c <<EOCP
 #include <sys/types.h>
 #include <stdio.h>
+#$i_stdlib I_STDLIB
...
+$rm -f try try.*
 ...
 : see what type of char stdio uses.
 echo " "
@@ -14043,6 +14752,10 @@ echo "Checking the size of $zzz..." >&4
 cat > try.c <<EOCP
 #include <sys/types.h>
 #include <stdio.h>
+#$i_stdlib I_STDLIB
...
+echo 'return 0;}' >> try.c
 ...
 cat <<EOSH >> Cppsym.try
 ccflags="$ccflags"
@@ -14438,7 +15165,7 @@ for i in \`$cc -v -c tmp.c 2>&1 $postprocess_cc_v\`
 do
 	case "\$i" in
 	-D*) echo "\$i" | $sed 's/^-D//';;
-	-A*) $test "$gccversion" && echo "\$i" | $sed 's/^-A\(.*\)(\(.*\))/\1=\2/';;
+	-A*) $test "$gccversion" && echo "\$i" | $sed 's/^-A//' | $sed 's/\(.*\)(\(.*\))/\1=\2/';;
 ...
 $rm -f try.c
@@ -14802,7 +15529,7 @@ find_extensions='
            else
                if $test -d $xxx -a $# -lt 10; then
                    set $1$xxx/ $*;
-                   cd $xxx;
+                   cd "$xxx";
                    ...
                    cd ..;
                    shift;
@@ -14812,17 +15539,21 @@ find_extensions='
        esac;
     done'
 tdir=`pwd`
-cd $rsrc/ext
