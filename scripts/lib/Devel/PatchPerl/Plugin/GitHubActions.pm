...
+$cat >try.c <<EOCP
 ...
 #ifdef I_FCNTL
 #include <fcntl.h>
@@ -8823,6 +9251,10 @@ $cat >open3.c <<'EOCP'
 #ifdef I_SYS_FILE
 #include <sys/file.h>
 #endif
+#$i_stdlib I_STDLIB
...
@@ -8845,10 +9277,10 @@ if $test `./findhdr sys/file.h` && \
 		val="$undef"
 	fi
 elif $test `./findhdr fcntl.h` && \
...
+$rm -f try try.*
 ...
 : see which of string.h or strings.h is needed
 echo " "
@@ -8885,6 +9317,35 @@ case "$i_string" in
 *)	  strings=`./findhdr string.h`;;
 esac
...
+
 ...
 case "$h_sysfile" in
 true) echo "#include <sys/file.h>" > head.c;;
@@ -8900,8 +9361,16 @@ echo "Figuring out the flag used by open() for non-blocking I/O..." >&4
 case "$o_nonblock" in
 '')
 	$cat head.c > try.c
-	$cat >>try.c <<'EOCP'
...
+	$cat >try.c <<EOCP
 ...
 #ifdef I_TIME
 #include <time.h>
@@ -9263,6 +9740,10 @@ if test "X$timeincl" = X; then
 #ifdef I_SYSSELECT
 #include <sys/select.h>
 #endif
+#$i_stdlib I_STDLIB
...
+#endif
 ...
 {
     struct sigaction act, oact;
@@ -11634,8 +12127,12 @@ $rm -f try try$_o try.c
 echo " "
 case "$d_sigsetjmp" in
 '')
-	$cat >try.c <<'EOP'
